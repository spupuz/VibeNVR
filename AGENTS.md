...
```
> **CRITICAL RULE**: AI Agents MUST NEVER run `docker compose` commands directly. Always ask the USER to run the rebuild commands manually.
...
- **Localization**: NEVER hardcode English strings in JSX. Always use the `t()` hook from `react-i18next` (e.g., `t('key', 'Default English')`). Use `extract_t.py` and `auto_translate.py` in `frontend/scripts/` to generate translations.
...
- **Rule**: When using `av.BitStreamFilterContext` (e.g., `h264_mp4toannexb`), the `.filter(packet)` method consumes/steals the C-level buffer of the original `av.Packet`. If the original packet is needed for a ring buffer or another thread, you MUST pass a copy (`av.Packet(packet)`) to the BSF, or else downstream consumers will silently receive empty (0-byte) packets, breaking muxing and decoding.
- **Rule**: PyAV's MP4 muxer requires packets to be in AVCC format (4-byte length prefixes). If a camera outputs raw Annex-B packets (e.g., OEM cameras relying on SDP SPS/PPS), feeding these directly to `container.mux()` will result in corrupted/black MP4 files because the `avcC` atom will contain Annex-B data instead of AVCC data. Passthrough MUST be disabled for these streams so they are transcoded, or a dedicated `mkv`/`ts` container must be used instead.
...
- **Testing**: Before any verification (automated or manual), always perform a full rebuild to ensure all changes are active. **DO NOT run this yourself; ask the USER to run it:**
  ```bash
