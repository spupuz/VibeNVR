...
async def _fetch_and_update_health(
    ...
    if previous_health is not None and previous_health != current_health:
        logger.info(
            f"Camera {camera.name} (ID: {camera.id}) health changed: {previous_health} -> {current_health}"
        )
        ...
        if current_health in ("UNREACHABLE", "UNAUTHORIZED", "OFFLINE"):
            ...
            previous_health in ("UNREACHABLE", "UNAUTHORIZED", "OFFLINE")
            and current_health == "CONNECTED"
        ):
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                try:
                    ...
                    if response.status_code != 200:
                        logger.warning(
                            f"Health check: Engine status returned {response.status_code}"
                        )
                        continue
                            ...
                            try:
                                with database.get_db_ctx() as db:
                                    ...
                                    if active_cams_count > 0:
                                        logger.info(
                                            f"Health check: Engine is empty but {active_cams_count} cameras should be active. Triggering automatic camera re-sync..."
                                        )

                                        def run_sync():
                                            try:
                                                with database.get_db_ctx() as db_inner:
                                                    motion_service.generate_motion_config(
                                                        db_inner
                                                    )
                                            except Exception as sync_e:
                                                logger.error(
                                                    f"Health check: Background re-sync failed: {sync_e}"
                                                )

                                        import threading

                                        threading.Thread(
                                            target=run_sync, daemon=True
                                        ).start()
                    ...
                    logger.warning(
                        "Health check: Cannot reach Engine (VibeEngine might be starting or stopped)"
                    )
                    continue
...
async def trigger_health_notification(db: Session, camera, status):
    ...
    if not (
        camera.notify_health_email
        or camera.notify_health_telegram
        or camera.notify_health_webhook
    ):
        return
