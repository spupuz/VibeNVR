...
| 🛡️ **Secure by Design** | Full JWT authentication, **SSO / OAuth (Auto-Redirect & RP-Logout)**, **2FA with Trusted Devices**, Rate Limiting, and HttpOnly Media Cookies. |
...
| 🌐 **Decentralized Federation** | **New**: Seamlessly link multiple VibeNVR instances together via API Proxies. Manage all your remote sites from a single "Master" pane of glass, complete with RBAC isolation and offline degradation fallbacks. |
...
| 💾 **Storage Management** | Automated cleanup (FIFO), **Reactive Quotas**, **Tiered Storage Archival**, **Storage Routing** (assign continuous/motion to different profiles), and Multiple Storage Profiles with detailed UI Breakdowns. |
...
2.  **Configuration (.env)**:
    ...
    **Important: Update your `.env` with secure values:**
    ...
    # .env content example
    # ENV Configuration
    SECRET_KEY=change_this_to_a_long_random_string  # CRITICAL: Security key (min 32 chars). If omitted, a dynamic key is generated, logging out users on every restart. Application won't start with an explicitly weak key.
