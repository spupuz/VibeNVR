...
1. **Path Traversal & SSRF Prevention**:
   ...
   - **DNS Rebinding & IPv6 Defense**: URL parsing utilizes `socket.getaddrinfo` to resolve all possible IP addresses (including IPv6 and multiple A records). Each resolved IP is rigorously checked against loopback, private, reserved, link-local, and multicast ranges before establishing a connection, preventing advanced SSRF bypass techniques.
...
8. **Secure Subprocess Execution**:
   - All internal calls to video tools (`ffmpeg`, `ffprobe`) are performed using **list-based arguments** (the secure default in Python's `subprocess.run`), effectively preventing any shell injection vulnerabilities via malicious camera URLs or paths.
    - **Advanced Log & GUI Masking**:
        ...
        - **Hardened Notification Pipeline**: All external notification dispatchers (Telegram, Webhooks, Email) implement mandatory sanitization. For Telegram, HTML parse mode is used with automated entity escaping to prevent malformed payload injection.
        - **Sanitized Exports**: Camera configuration exports (JSON/TAR) are strictly sanitized. Hashed secrets and credentials are removed or masked before the archive is generated, and the export process is protected by a dedicated security audit on every run.
...
- **Webhook SSRF Blocking**: The webhook validation strictly blocks all private IP ranges, loopback (`127.0.0.1`), link-local, and unspecified IPs. This prevents targeting internal containers (like the database or engine) or local services on the host network, closing internal service SSRF vulnerabilities.
