...
- **Localization (i18n)**: All UI strings are decoupled from JSX using `react-i18next`. Translation dictionaries for 10 supported languages are auto-generated via Python scripts (`frontend/scripts/extract_t.py` and `auto_translate.py`).
...
- **Temporary Files & Tests**: Any temporary script, test file, or scratchpad created during the session must be deleted and NOT committed, unless it is placed specifically within the designated local folders (`scripts/security_tests/` or `scripts/tests/`) which are explicitly git-ignored.
